...
public class ContentHistoryController {
	...
	@GetMapping
	public ResponseEntity<ResponseDto<List<ContentHistory>>> getMyContentHistories(@PathVariable Long memberId) {
		ResponseDto<List<ContentHistory>> contentHistories = contentHistoryService.getMyContentHistories(memberId);
		return new ResponseEntity<>(contentHistories, HttpStatus.OK);
	}
